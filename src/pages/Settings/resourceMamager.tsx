...
export const ResourceManagerScreen: React.FC = () => {
    ...
    const [offlinePriority, , toogleOfflinePriority] = useSetting(OFFLINE_PRIORITY_KEY)
    ...
    return (
        <ScreenContainer minimal lowerMargin>
            <ul>
                ...
                >
                    {offline ? <MdDone size={30} /> : <MdClose size={30} />}
                    <div className="pl-1">
                ...
                <li onClick={toogleOfflinePriority} className="flex w-screen items-center hover:bg-app-200 hover:text-app-900 pl-3 py-1.5 duration-300">
                    {offlinePriority ? <MdDone size={30} /> : <MdClose size={30} />}
                    <div className="pl-1">
                        <span>Prioridade para Recursos Offline</span>
                        <br />
                        <span className="text-sm">Mesmo quando você estiver com internet o sistema vai priorizar os recursos do servidor de tunelamento offline.</span>
                    </div>
                </li>
            </ul>
        </ScreenContainer>
